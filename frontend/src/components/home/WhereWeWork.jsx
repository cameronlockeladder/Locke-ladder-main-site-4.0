        ...
        {data.regions.map((r, i) => (
          <Reveal key={r.region} delay={i * 0.06}>
            <div className="border-t border-line pt-8">
              <h3 className="font-display text-2xl font-medium tracking-tight text-paper md:text-3xl">{r.region}</h3>
              ...
              <ul className="mt-7 grid grid-cols-2 gap-x-6 gap-y-3">
                {r.cities.map((c) => (
                  <li key={c} className="text-base text-paper/70 border-b border-line/60 pb-2">{c}</li>
                ))}
              </ul>
            </div>
          </Reveal>
